...
class CustomIntroductionScreenState extends State<CustomIntroductionScreen> {
  ...
  void initState() {
    ...
    if(widget.autoScroll)
      {
        Timer.periodic(widget.autoScrollDuration,(_){
          if(_pageController.page?.round()==widget.pages!.length - 1)
            {
              _pageController.jumpToPage(0);
            }
          else
            {
              _pageController.nextPage(duration: Duration(milliseconds: widget.animationDuration), curve: widget.curve);
            }
